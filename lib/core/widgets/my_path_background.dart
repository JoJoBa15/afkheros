...
// NOTA: Rinominato da ParallaxBackground a MyPathBackground per compatibilità
...
class _MyPathBackgroundState extends State<MyPathBackground>
  ...
  Widget build(BuildContext context) {
    // Crossfade per miniera (usa i tuoi 4 PNG)
    final mineBlend = _computeBlend(_now);

    // Intensità “notte” (0..1) e altri effetti
            ...
            builder: (_, __) {
              ...
              return Stack(
                ...
                children: [
                  ...
                  // 2) Sole/Luna (procedurale, 0 frame)
                  CustomPaint(
                  ...
                  // 3) Nuvole (1 sprite che scorre, 0 frame)
                  // Commentato perché 'cloud_01.png' non è presente negli asset
                  /* 
                  Opacity(
                  ...
                  */
                  ...
                  // 4) Miniera (le tue immagini, appoggiate in basso)
                  Opacity(
                    opacity: 1 - smoothstep(mineBlend.t),
                    child: Image.asset(mineBlend.a, fit: BoxFit.fill, filterQuality: FilterQuality.none),
                  ),
                  Opacity(
                    opacity: smoothstep(mineBlend.t),
                    child: Image.asset(mineBlend.b, fit: BoxFit.fill, filterQuality: FilterQuality.none),
                  ),

                  // 5) OVERLAYS (reintrodotti dalla versione precedente)
                  // Nebbia mattutina
                  if (mist > 0.001)
                  ...
                  if (nightness > 0.001)
                    CustomPaint(painter: LanternGlowPainter(t: t, intensity: nightness)),
                ],
              );
            },
          ),
...
class PixelSkyPainter extends CustomPainter {
  ...
  PixelSkyPainter({required this.now, required this.parallax});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < bands; i++) {
      ...
      final rect = Rect.fromLTWH(0, y0, size.width, (y1 - y0).abs() + 2);
      canvas.drawRect(rect, Paint()..color = c..isAntiAlias = false);
    }

    final night = _nightness(now);
    if (night > 0.05) {
      final p = Paint()..color = Color.fromARGB((80 * night).round(), 255, 255, 255)..isAntiAlias = false;
      for (int i = 0; i < 60; i++) {
        final x = _hash1(i * 97) * size.width;
        final y = _hash1(i * 53) * (size.height * 0.55);
        canvas.drawRect(Rect.fromLTWH(x.roundToDouble(), y.roundToDouble(), 2, 2), p);
      }
    }
  }
...
_SkyScheme _skyScheme(DateTime now) {
  final m = now.hour * 60 + now.minute;
  if (m >= 6 * 60 && m < 8 * 60) return const _SkyScheme(Color(0xFF2B2D6E), Color(0xFFFFB36A)); // sunrise
  if (m >= 8 * 60 && m < 17 * 60) return const _SkyScheme(Color(0xFF4CA7FF), Color(0xFFBFE8FF)); // day
  if (m >= 17 * 60 && m < 20 * 60 + 30) return const _SkyScheme(Color(0xFF3A2C6E), Color(0xFFFF7A59)); // sunset
...
class SunMoonPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final center = isDaySun ? _sunPos(size, m, rise, set, parallax) : _moonPos(size, m, parallax);
    final night = _nightness(now);
    ...
    if (isDaySun) {
      ...
      final glow = Paint()
        ...
        ..shader = RadialGradient(
          colors: [const Color(0x66FFD27A), const Color(0x00FFD27A)],
        ).createShader(Rect.fromCircle(center: center, radius: r * 4));

      canvas.drawCircle(center, r * 4, glow);
      canvas.drawRect(Rect.fromLTWH((center.dx - r).roundToDouble(), (center.dy - r).roundToDouble(),
          (2 * r).roundToDouble(), (2 * r).roundToDouble()),
      ...
      final paint = Paint()..color = Color.fromARGB(alpha, 220, 230, 255)..isAntiAlias=false;
      canvas.drawRect(Rect.fromLTWH((center.dx - r).roundToDouble(), (center.dy - r).roundToDouble(),
          (2 * r).roundToDouble(), (2 * r).roundToDouble()), paint);
    }
  }

  Offset _sunPos(Size size, int m, int rise, int set, double parallax) {
    final t = ((m - rise) / (set - rise)).clamp(0.0, 1.0);
    final x = lerpDouble(-0.1, 1.1, t)! * size.width;
    final peak = size.height * 0.18;
    final base = size.height * 0.42;
    final y = (base - math.sin(math.pi * t) * (base - peak)) + parallax * 6;
...
class LanternGlowPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rOuter = size.width * 0.22, rInner = size.width * 0.08;
    
    final paint = Paint()
      ..blendMode = BlendMode.screen
      ..shader = RadialGradient(
        colors: [Color.lerp(const Color(0x00FFD08A), const Color(0xAAFFB05A), a)!, const Color(0x00000000)],
        stops: const [0.0, 1.0],
...
class MistPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < blobs; i++) {
      final p = _hash1(i * 97), q = _hash1(i * 193);
...
// NOTA: rinominata da _hash01 a _hash1 per evitare duplicati
double _hash1(int n) {
